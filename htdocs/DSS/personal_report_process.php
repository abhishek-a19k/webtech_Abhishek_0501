...
<?php
    ...
    $req_field = array('username');

    validate_fields($req_field);

    if(empty($errors)):
        $name   = remove_junk($db->escape($_POST['username']));
        $results = find_personal_history($name);

    else:
        $session->msg("d", $errors);
        redirect('personal_report.php', false);
    endif;

    } else {
    $session->msg("d", "Select user");
    redirect('personal_report.php', false);
    }
?>
